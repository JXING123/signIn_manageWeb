	...
	<head>
		...
		<script type="text/javascript">
						...
						series: {  
            				...
            				events: {  
                				click: function(e) {  
									...
									window.location.href="classStatistics?departmentId="+departmentId+"&startTime="+startDate+"&endTime="+endDate+"&className="+e.point.category;
                				}  
                			}
            			}  
					},
			...
			function getData(){  
        		...
        		$.ajax({
	            	type : "POST",
	            	url : "getDIClassAttendanceStatus",
	            	dataType : "json",
	            	success : function(result) {
	            		$(result).each(function(n,item){
        					...
        					var classCount = "<tr><td>"+item.className+"</td><td>"+item.zhengChang+"</td><td>"+item.chiDao+"</td><td>"+item.zaoTui+"</td><td>"+item.kuangKe+"</td></tr>";
			...
			function getClassTotalData(){  
        		...
        		$.ajax({
	            	type : "POST",
	            	url : "getDIClassAttendanceStatus?total=1",
	            	dataType : "json",
	            	success : function(result) {
	            		var data = [['正常',result.zhengChang],['早退', result.zaoTui],['迟到', result.chiDao],{
       					...
       					var diName = $("#departmentId option:selected").text();
       					$("#h2").html(diName+"各班级考勤总汇表");
       					var title = {
    						text:diName+"班级总体考勤百分比",
						};
		...
		<script type="text/javascript">
			function getCount(){
        		...
        		if(startDate=='' || endDate==''){
					...
					window.location.href="classAttendanceStatistics?departmentId="+departmentId+"&startTime="+startDate+"&endTime="+endDate;
        		}
        	}
        	function getTeacherCount(){
        		var departmentId = $("#departmentId").val();
        		window.location.href= "classTeacherInStatistics?departmentId="+departmentId;
        	}
        </script>
	</head>
				...
				<div class="box col-md-12">
					<div class="box-inner">
						<div class="box-header well" data-original-title="">
							<h2><i class="glyphicon glyphicon-edit"></i><b id="diName"></b>上课到勤统计</h2>
						</div>
						<div class="box-content">
								<div class="form-group">
				            		<select name="departmentId" id="departmentId" class="btn btn-default btn-sm" onchange="">
				            		 ...
				            		 开始时间：<input name="startTime" required id="startDate" placeholder="请输入日期" class="btn laydate-icon">
				            		  结束时间：<input name="endTime" required id="endDate" placeholder="请输入日期" class="btn laydate-icon">
							...
							<div class="controls">
								<table class="table table-striped table-bordered bootstrap-datatable datatable responsive" id="table2">
									...
									<thead>
										<tr>
											<th rowspan="2" style="vertical-align:middle; text-align:center;">班级</th>
